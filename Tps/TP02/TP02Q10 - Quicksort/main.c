...
void quicksortRec(Pokemon *array, int esq, int dir){
    ...
    while (i <= j) {
        comp++;
        while (getGeneration(&array[i]) < getGeneration(&pivo)||
              (getGeneration(&array[i]) == getGeneration(&pivo) && strcmp(getName(&array[i]), getName(&pivo)) < 0)) i++;
        comp++;
        while (getGeneration(&array[j]) > getGeneration(&pivo)||
              (getGeneration(&array[j]) == getGeneration(&pivo) && strcmp(getName(&array[j]), getName(&pivo)) > 0)) j--;
        if (i <= j) {
            swap(array, i, j);
            i++;
            j--;
