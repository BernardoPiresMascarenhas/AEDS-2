...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("matrícula_QuicksortPARCIAL.txt"))) {
...
class Sort {
    ...
    private static void quicksort(int esq, int dir, ArrayList<Pokemon> lista) {
        int i = esq, j = dir;
        Pokemon pivo = lista.get((dir+esq)/2);
        while (i <= j) {
            Main.comp++;
            while (lista.get(i).getGeneration() < pivo.getGeneration() ||
                  (lista.get(i).getGeneration() == pivo.getGeneration() && lista.get(i).getName().compareTo(pivo.getName()) < 0)) {
                i++;
            }
            Main.comp++;
            while (lista.get(j).getGeneration() > pivo.getGeneration() ||
                  (lista.get(j).getGeneration() == pivo.getGeneration() && lista.get(j).getName().compareTo(pivo.getName()) > 0)) {
            j--;
            }
        ...
        if (i < dir && i < k)  quicksort(i, dir, lista);
    }
