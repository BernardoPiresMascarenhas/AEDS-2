...
int main () {
    ...
    FILE *arquivo = fopen("matrícula_heapsortPARCIAL.txt.", "w");
    if (arquivo == NULL) {
        perror("Erro ao abrir o arquivo");
        return 1;
    }
    fprintf(arquivo, "833392\t%.2fms\t%dcomp", executionTime, comp);
    fclose(arquivo);
...
void reconstruir(Pokemon *array, int tamHeap){
    ...
    while(i <= (tamHeap/2)){
        ...
        comp+=3;
        if( ( getHeight(&array[i]) < getHeight(&array[filho]) ) ||
            ( getHeight(&array[i]) == getHeight(&array[filho]) &&
              strcmp(getName(&array[i]), getName(&array[filho])) < 0 ) ) {
            swap(i, filho, array);
            i = filho;
        }else{
            i = tamHeap;
...
int getMaiorFilho(Pokemon *array, int i, int tamHeap){
    int filho;
    comp++;
    if (2*i == tamHeap || getHeight(&array[2*i]) > getHeight(&array[2*i+1])){
        filho = 2*i;
    } else {
        comp+=3;
        if ((getHeight(&array[2 * i]) > getHeight(&array[2 * i + 1])) ||
            (getHeight(&array[2 * i]) == getHeight(&array[2 * i + 1]) &&
             strcmp(getName(&array[2 * i]), getName(&array[2 * i + 1])) > 0)) {
            filho = 2 * i;
        } else {
            filho = 2 * i + 1;
        }
...
void construir(Pokemon *array, int tamHeap){
    for (int i = tamHeap; i > 1 && 
        (getHeight(&array[i]) > getHeight(&array[i / 2]) || 
        (getHeight(&array[i]) == getHeight(&array[i / 2]) && 
         strcmp(getName(&array[i]), getName(&array[i / 2])) > 0)); i /= 2) {
        comp++;
        swap(i, i / 2, array);
    }
}
