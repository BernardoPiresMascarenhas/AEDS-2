...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < entrada.size(); i++) {
			Pokemon tmp = entrada.get(i);
            int j = i - 1;

            comp++;
            while ((j >= 0) && (entrada.get(j).getCaptureDate().compareTo(tmp.getCaptureDate()) >= 0)) {
                comp++;
                if(entrada.get(j).getCaptureDate().compareTo(tmp.getCaptureDate()) == 0){
                    comp++;
                    if(entrada.get(j).getName().compareTo(tmp.getName()) > 0){
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("matrícula1_insercao.txt"))) {
