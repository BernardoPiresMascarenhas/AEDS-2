...
void insercao(Pokemon *array, int n);
...
int main () {
    ...
    FILE *arquivo = fopen("matrícula_insercaoPARCIAL.txt.", "w");
    if (arquivo == NULL) {
...
void insercao(Pokemon *array, int n){
    for (int i = 1; i < n; i++) {
        ...
        while (j >= 0 && 
              ((getCaptureDate(&array[j]).ano > getCaptureDate(&tmp).ano) || 
               (getCaptureDate(&array[j]).ano == getCaptureDate(&tmp).ano && 
                getCaptureDate(&array[j]).mes > getCaptureDate(&tmp).mes) || 
               (getCaptureDate(&array[j]).ano == getCaptureDate(&tmp).ano && 
                getCaptureDate(&array[j]).mes == getCaptureDate(&tmp).mes && 
                getCaptureDate(&array[j]).dia > getCaptureDate(&tmp).dia))) {
            array[j + 1] = array[j];
            j--;
            comp++;
        }
