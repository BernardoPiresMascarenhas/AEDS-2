...
int pesquisa_binaria(Pokemon *array, int tamanho, char name[]);
...
int main () {
    ...
    while(strcmp(name, "FIM") != 0) {
        int resultado = pesquisa_binaria(array, n, name);
        if(resultado != -1) {
            printf("SIM\n");
        } else {
            printf("NAO\n");
...
int pesquisa_binaria(Pokemon *array, int tamanho, char name[]) {
    ...
    while (esquerda <= direita) {
        ...
        // Verifica se o valor procurado está no meio
        comp++;
        if (strcmp(getName(&array[meio]), name) == 0) {
            return meio; // Retorna o índice onde o valor foi encontrado
        }

        // Se o valor procurado for maior, ignore a metade esquerda
        comp++;
        if (strcmp(getName(&array[meio]), name) < 0) {
            esquerda = meio + 1;
        }
        // Se o valor procurado for menor, ignore a metade direita
        else {
            direita = meio - 1;
        }
    }

    // Retorna -1 se o valor não foi encontrado
    return -1;
}
